...
# Function to get highest number from spec directories on each remote's default
# branch. Merged feature branches get deleted, so a spec dir that exists only on
# origin/main is invisible to both the local specs scan (stale checkout) and the
# branch scans — which is exactly how two specs ended up numbered 218. Reads the
# last-fetched remote refs (side-effect-free; callers fetch when appropriate).
get_highest_from_remote_specs() {
    local highest=0

    for remote in $(git remote 2>/dev/null); do
        local default_ref
        default_ref=$(git symbolic-ref --quiet --short "refs/remotes/$remote/HEAD" 2>/dev/null || echo "$remote/main")
        local remote_highest
        remote_highest=$(git ls-tree -d --name-only "$default_ref" -- specs/ 2>/dev/null | sed 's|^specs/||' | _extract_highest_number)
        if [ "$remote_highest" -gt "$highest" ]; then
            highest=$remote_highest
        fi
    done
...
check_existing_branches() {
    ...
    local highest_remote_spec=$(get_highest_from_remote_specs)

    local max_num=$highest_branch
    if [ "$highest_spec" -gt "$max_num" ]; then
        max_num=$highest_spec
    fi
    if [ "$highest_remote_spec" -gt "$max_num" ]; then
        max_num=$highest_remote_spec
    fi
